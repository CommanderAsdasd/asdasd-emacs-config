...
(setq ELS (expand-file-name (concat user-emacs-directory "els")))
...
(defun asdasd/signature-check-mode (&optional arg)
  ...
  (print (symbol-value package-check-signature))
  )
...
(use-package modular-config
  :config
  (setq modular-config-path ELS)
  (setq modular-config-list '(
                              (all (ui mod-completion-ivy snippets mod-requires-deprec))
                              (helm ((all mod-completion-ivy) mod-completion-helm)
                              ))
  (setq modular-config-default 'all)
  (modular-config-command-line-args-process) ;;this processes command line when met --config minimal
  ))
...
(custom-set-variables
 ...
 '(magit-git-executable "c:/msys64/usr/bin/git.exe")
