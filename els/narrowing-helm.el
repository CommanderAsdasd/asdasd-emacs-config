;; ;;; narrowing-helm.el --- My startup file for helm. -*- lexical-binding: t -*-
;; ;;; Code:
...
;; ;; do not break incapsulation - function should not know anything about outer world functions - so don do it's behaviour dependant on type of helm action

(defun asdasd-helm-kill-entry (candidate)
  "helm-find-files-actions"
  (print candidate)
  (kill-new candidate)
  )

;; ;; (defun asdasd-helm-kill-command-name (&rest candidate)
;; ;;   "wrapper of helm-find-files-actions"
;; ;;   ( (car candidate))
;; ;;   ;; (let* ((candidate-name (car candidate)))
;; ;;   ;;   (asdasd-helm-kill-entry candidate-name))
;; ;;   )

;; (helm-add-action-to-source "kill entry" 'asdasd-helm-kill-entry 'helm-browse-project-build-buffers-source)

;; ;; (add-to-list 'helm-find-files-actions '("kill entry" . asdasd-helm-kill-entry))
;; ;; (add-to-list 'helm-type-buffer-actions '("kill entry" . asdasd-helm-kill-entry)) 
;; ;; (add-to-list 'helm-type-command-actions '("kill command name" . asdasd-helm-kill-command-name))

;; (add-hook 'minibuffer-inactive-mode-hook '(local-set-key (kbd "M-q c") 'helm-regexp))
...
;;   :ensure t
;;   :config (setq ace-jump-helm-line-style 'pre)
;;   (setq ace-jump-helm-line-background t)
;;   (setq ace-jump-helm-line-default-action 'select)
;; ;;; init-h(setq ace-jump-helm-line-select-key ?e) ;; this line is not neeededelm.el ends here
...
;;   (general-add-hook 'helm-major-mode-hook #'general-define-key      
;;                     "M-q w" 'ace-jump-helm-line))
...
(provide 'asdasd-completion-helm)
